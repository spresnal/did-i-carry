...
import '/server/api/leagueApis/matchApi.js';
...
describe('matchApi', function () {
    ...
    describe('getMatchById', function () {
        it('succesfully builds the url', function () {
            // act
            Meteor.call('getMatchById', {matchId: 1337});

            // assert
            expect(stub.calledOnce).to.be.true;
            expect(stub.getCall(0).args[0]).to.equal('GET');
            expect(stub.getCall(0).args[1]).to.equal('https://na.api.pvp.net/api/lol/na/v2.2/match/1337?api_key=undefined');
        });
    });
});